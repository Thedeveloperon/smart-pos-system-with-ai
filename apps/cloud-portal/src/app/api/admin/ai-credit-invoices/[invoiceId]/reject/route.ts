...
export async function POST(request: NextRequest, context: RouteParams) {
  ...
  const actorNote = typeof candidate.actor_note === "string" ? candidate.actor_note.trim() : "";
  if (!actorNote) {
    return toInvalidResponse("actor_note is required.");
  }

  const reasonCode = typeof candidate.reason_code === "string" ? candidate.reason_code.trim() : "";
  const sanitizedPayload: Record<string, string> = {
    actor_note: actorNote,
  };
  if (reasonCode) {
    sanitizedPayload.reason_code = reasonCode;
  }
  ...
  return forwardUpstreamRequest({
    ...
    backendPath: `/api/admin/licensing/ai-credit-invoices/${encodeURIComponent(invoiceId)}/reject`,
