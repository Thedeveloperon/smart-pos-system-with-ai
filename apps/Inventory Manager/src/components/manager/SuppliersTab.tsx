...
export default function SuppliersTab() {
  ...
  const handleSave = async () => {
    ...
    const emailError = editorMode === "extended" ? getOptionalEmailError(supplierForm.email) : null;
    if (emailError) {
  ...
  const handleSupplierAction = async () => {
    ...
    try {
      if (actionState.mode === "activate") {
        ...
        setSuppliers((prev) =>
          prev.filter((item) => item.supplier_id !== actionState.supplier.supplier_id),
        );
    ...
    <>
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <Table
            loading={loading}
            emptyText={searchQuery.trim() ? "No sales reps match your search." : "No sales reps found."}
            ...
            <>
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="grid gap-1.5">
                  ...
                  />
                  {emailError ? (
                    <p id="supplier-email-error" className="text-sm text-destructive">
                      {emailError}
                    </p>
                  ) : null}
                </div>
