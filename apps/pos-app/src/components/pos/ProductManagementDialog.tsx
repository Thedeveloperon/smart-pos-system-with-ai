...
function snapshotProduct(
  ...
  return {
    ...
    permanentDiscountPercent: form.permanentDiscountPercent === "" ? null : toNumber(form.permanentDiscountPercent),
    permanent_discount_percent: form.permanentDiscountPercent === "" ? null : toNumber(form.permanentDiscountPercent),
    permanentDiscountFixed: form.permanentDiscountFixed === "" ? null : toNumber(form.permanentDiscountFixed),
...
export default function ProductManagementDialog({
  ...
  const [adjustQuantityError, setAdjustQuantityError] = useState(false);
  const [adjustReason, setAdjustReason] = useState("");
  const [adjustReasonError, setAdjustReasonError] = useState(false);
  ...
  useEffect(() => {
    ...
    setDeleteMode(null);
    setAdjustQuantity("0");
    setAdjustQuantityError(false);
    setAdjustReason("");
    setAdjustReasonError(false);
    setAdjustBatchId("");
                  ...
                  <SelectContent>
                    ...
                    {categories.map((item) => (
                      <SelectItem
                        ...
                        disabled={!item.is_active && form.categoryId !== item.category_id}
                        className={!item.is_active ? "text-amber-700" : undefined}
                      >
                        {item.name}
          ...
          <div className="grid gap-4">
            <div className="grid gap-1.5">
              ...
              <Input
                ...
                className={adjustQuantityError ? "border-destructive focus-visible:ring-destructive" : undefined}
                onChange={(event) => {
                  const nextValue = event.target.value;
                  setAdjustQuantity(nextValue);
                  if (adjustQuantityError && toNumber(nextValue) !== 0) {
                    setAdjustQuantityError(false);
                  }
                }}
              />
            ...
            <div className="grid gap-1.5">
              ...
              <Textarea
                ...
                className={adjustReasonError ? "border-destructive focus-visible:ring-destructive" : undefined}
                onChange={(event) => {
                  const nextValue = event.target.value;
                  setAdjustReason(nextValue);
                  if (adjustReasonError && nextValue.trim()) {
                    setAdjustReasonError(false);
                  }
                }}
                rows={3}
