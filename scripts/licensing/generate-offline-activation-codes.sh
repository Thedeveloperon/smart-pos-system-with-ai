...
BACKEND_URL="${SMARTPOS_BACKEND_URL:-http://127.0.0.1:5080}"
...
SHOP_CODE="${SMARTPOS_SHOP_CODE:-}"
COUNT="${SMARTPOS_BATCH_COUNT:-1}"
...
ACTOR_NOTE="${SMARTPOS_BATCH_ACTOR_NOTE:-manual offline activation code generation}"

if ! [[ "$COUNT" =~ ^[0-9]+$ ]] || (( COUNT < 1 || COUNT > 10 )); then
  echo "SMARTPOS_BATCH_COUNT must be between 1 and 10. Current value: $COUNT" >&2
  exit 1
fi
...
LOGIN_PAYLOAD="$(jq -n \
  ...
  '{
    ...
    } + (if ($mfa_code | length) > 0 then { mfa_code: $mfa_code } else {} end)')"
...
if [[ "$LOGIN_STATUS" -lt 200 || "$LOGIN_STATUS" -ge 300 ]]; then
  ...
  echo "Tip: verify backend URL ($BACKEND_URL) and admin credentials." >&2
...
BATCH_PAYLOAD="$(jq -n \
  ...
  '{
    ...
    } + (if ($shop_code | length) > 0 then { shop_code: $shop_code } else {} end)')"
...
if [[ "$(jq -r '.generated_count' "$API_BODY_FILE")" == "1" ]]; then
  echo "Offline activation code generated successfully."
else
