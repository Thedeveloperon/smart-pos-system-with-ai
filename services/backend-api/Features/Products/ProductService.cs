    ...
    PromotionService promotionService)
{
    ...
    private const string BrandDeactivateLinkedRecordsMessage = "Cannot deactivate brand while linked to active products or active suppliers. Unlink related products/categories/suppliers first.";
    ...
    public async Task HardDeleteInactiveProductAsync(Guid productId, CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException(
                "This product is linked to other records and cannot be permanently deleted.");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var hasLinkedActiveProducts = await dbContext.Products
                .AsNoTracking()
                .AnyAsync(
                    x => x.BrandId == brandId &&
                         x.IsActive &&
            ...
            var hasLinkedActiveSuppliers = await dbContext.SupplierBrands
                .AsNoTracking()
                .AnyAsync(
                    x => x.BrandId == brandId &&
                         x.Supplier.IsActive &&
            ...
            if (hasLinkedActiveProducts || hasLinkedActiveSuppliers)
            {
                throw new InvalidOperationException(BrandDeactivateLinkedRecordsMessage);
            }
        }
