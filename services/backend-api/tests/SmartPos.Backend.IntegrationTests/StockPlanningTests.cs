    ...
    : IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task SupplierHardDelete_ShouldRejectSupplierWithPurchaseOrders()
    {
        ...
        var createPurchaseOrderResponse = await client.PostAsJsonAsync("/api/purchase-orders", new
        {
            supplier_id = supplierId,
            po_number = $"PO-LINK-{runId}",
            ...
            lines = new[]
            {
                new
                {
                    ...
                    quantity_ordered = 3m,
                    unit_cost_estimate = 60m
                }
            }
        });
        Assert.Equal(HttpStatusCode.OK, createPurchaseOrderResponse.StatusCode);
        ...
        var errorPayload = JsonNode.Parse(await hardDeleteResponse.Content.ReadAsStringAsync())?.AsObject()
                           ?? throw new InvalidOperationException("Response body was empty.");
        Assert.Equal(
            "This supplier has purchase order history and cannot be permanently deleted.",
            TestJson.GetString(errorPayload, "message"));
    }
    ...
    public async Task BrandHardDelete_ShouldAllowInactiveBrandAfterLinkedProductAndSupplierAreInactive()
    {
        ...
        var brand = await TestJson.ReadObjectAsync(
            await client.PostAsJsonAsync("/api/brands", new
            {
                name = $"Inactive Linked Brand {runId}",
                code = $"ILB-{runId}",
                is_active = true
            }));
        var brandId = Guid.Parse(TestJson.GetString(brand, "brand_id"));
        ...
        var deactivateBrand = await TestJson.ReadObjectAsync(
            await client.PutAsJsonAsync($"/api/brands/{brandId}", new
            {
                name = $"Inactive Linked Brand {runId}",
                code = $"ILB-{runId}",
        ...
        Assert.False(deactivateBrand["is_active"]?.GetValue<bool>() ?? true);
