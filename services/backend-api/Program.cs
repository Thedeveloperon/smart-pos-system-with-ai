...
using SmartPos.Backend.Features.Bundles;
...
using SmartPos.Backend.Features.Import;
...
using SmartPos.Backend.Features.Services;
    ...
    CancellationToken cancellationToken)
{
    ...
    for (var attempt = 1; attempt <= maxAttempts; attempt++)
    {
        try
        {
            ...
            await DbSchemaUpdater.EnsureDiscountSchemaAsync(dbContext);
            await DbSchemaUpdater.EnsurePackAndBundleSchemaAsync(dbContext);
            await DbSchemaUpdater.EnsureServiceSchemaAsync(dbContext);
