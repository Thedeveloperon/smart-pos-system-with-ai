...
- [x] Gap 1: TypeScript type `AiPendingManualPaymentItem` added to `page.tsx`
- [x] Gap 1: Six new state variables added to `page.tsx`
- [x] Gap 1: `Promise.all` in `loadAiBillingData` extended to fetch pending-manual
- [x] Gap 1: `handleVerifyAiManualPayment` callback added
- [x] Gap 1: `handleVerifyAiManualPaymentByReference` callback added
- [x] Gap 1: Pending Verifications card JSX added inside AI credits section
- [x] Gap 2: `HeaderBarProps` has `isAiCreditLow` and `cloudPortalUrl`
- [x] Gap 2: Desktop badge turns amber with "!" when low
- [x] Gap 2: "Top Up" anchor rendered beside/below desktop badge when low
- [x] Gap 2: Mobile dropdown badge turns amber with "Low" label
- [x] Gap 2: `<HeaderBar>` in `Index.tsx` passes both new props
